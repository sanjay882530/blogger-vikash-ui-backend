...
app.post('/api/signup', async (req, res) => {
    ...
    try {
        ...
        const [erows] = await promisePool.query('SELECT * FROM users WHERE email = ?', [email]);
        if (erows.length > 0) {
        ...
        // Insert new user
        await promisePool.query('INSERT INTO users (username, password,email,role) VALUES (?, ?, ?, ?)', [username, hashedPassword, email,'Blogger']);
...
app.post("/api/getUserId", async (req, res) => {
    ...
    try {
        const [rows] = await promisePool.query('SELECT id FROM users WHERE username = ?', [username]);
        if (rows.length > 0) {
...
app.post("/api/addBlog", async (req, res) => {
    const { title, description, full_description, author, image_url, user_id } = req.body;
    try {
